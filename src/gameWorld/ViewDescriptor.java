...
public class ViewDescriptor {
	...
	/**
	 * Constructs a new view descriptor object.
	 * @param p
	 */
	...
	private void generate(Player p, Board b) {
		...
		switch (dir) {
			case "north" :
				//add the centre tile
				if (b.getBoard()[y-1][x].hasWall(dir)) view.add("wall");
				else if(b.getBoard()[y-1][x].hasDoor(dir)) view.add("door");
				else view.add("open");
				view.add("wall");
				...
				for (int i = 0; i < 3; i++) {
					view.add(b.getBoard()[y-1][x-1 + i].hasItem() ?
							b.getBoard()[y-1][x-1 + i].getFloorObject().getName(): "clear");
				}
				break;
			case "east" :
				//add the centre tile
				if (b.getBoard()[y][x+1].hasWall(dir)) view.add("wall");
				else if(b.getBoard()[y][x+1].hasDoor(dir)) view.add("door");
				else view.add("open");
				view.add("wall");
				...
				for (int i = 0; i < 3; i++) {
					view.add(b.getBoard()[y-1 + i][x+1].hasItem() ?
							b.getBoard()[y-1 + i][x+1].getFloorObject().getName(): "clear");
				}
				break;
			case "south" :
				//add the centre tile
